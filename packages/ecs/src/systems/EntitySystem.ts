...
import Query, { InferQuerySet } from '../queries/Query';
import System from './System';

export type Infer<T> = T extends EntitySystem ? InferQuerySet<T["query"]> : never;
export default abstract class EntitySystem extends System  {
	...
	*/
	...
	override onEnable(): void {
		...
		const removeComponent = this.scope.eventbus.on("componentremoved", (entity: Entity) => {
			if (this.query.matches(entity))
				return;
			this._entities.delete(entity.id);
			this.onEntityRemoved?.(entity);
		});

		const entityDestroyed = this.scope.eventbus.on("entitydestroyed", (entity: Entity) => {
			this._entities.delete(entity.id);
			this.onEntityRemoved?.(entity);
		});

		const entityAdded = this.scope.eventbus.on("entitycreated", (entity: Entity) => {
		...
		const componentAdded = this.scope.eventbus.on("componentadded", (entity: Entity) => {
			if (!this.query.matches(entity) || this._entities.has(entity.id))
				return;
			this._entities.add(entity.id);
			this.onEntityAdded?.(entity);
